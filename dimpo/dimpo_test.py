# -*- coding: utf-8 -*-
"""
DIMPO — all tests in one file.

Covers: distances, probabilities, cost, gradients (numerical check), optimizer, DIMPO class.
"""
...
from dimpo.dimpo_core import (poisson_kl_pairwise, compute_distance_matrix, compute_S, compute_Q,
                               hellinger_distance, group_lasso_penalty, compute_cost,
                               compute_gradient, DIMPO)
...
# ====================================================================
# DIMPO CLASS (end-to-end)
# ====================================================================

def test_dimpo_fit_transform_shape():
    X, _ = _generate_synthetic_poisson(n_groups=2, n_conds_per_group=15, n_neurons=10)
    model = DIMPO(n_components=3, max_iter=50, verbose=False, eta=50.0,
...
def test_dimpo_cost_decreases():
    X, _ = _generate_synthetic_poisson(n_groups=2, n_conds_per_group=15, n_neurons=10)
    model = DIMPO(n_components=2, max_iter=100, verbose=False, eta=50.0,
                  use_momentum=False, use_early_exaggeration=False)
    model.fit(X)
...
def test_dimpo_with_momentum():
    X, _ = _generate_synthetic_poisson(n_groups=2, n_conds_per_group=10, n_neurons=8)
    emb = DIMPO(n_components=2, max_iter=50, verbose=False, eta=50.0,
...
def test_dimpo_with_early_exaggeration():
    X, _ = _generate_synthetic_poisson(n_groups=2, n_conds_per_group=10, n_neurons=8)
    emb = DIMPO(n_components=2, max_iter=50, verbose=False, eta=50.0,
...
def test_dimpo_with_gamma():
    X, _ = _generate_synthetic_poisson(n_groups=2, n_conds_per_group=10, n_neurons=8)
    emb = DIMPO(n_components=2, max_iter=50, verbose=False, eta=50.0, gamma=0.01,
...
def test_dimpo_perplexity_mode():
    X, _ = _generate_synthetic_poisson(n_groups=2, n_conds_per_group=15, n_neurons=8)
    emb = DIMPO(n_components=2, s_mode='perplexity', perplexity=5.0,
                max_iter=50, verbose=False, eta=50.0,
                use_momentum=False, use_early_exaggeration=False).fit_transform(X)
    assert np.all(np.isfinite(emb)), 'Embedding contains NaN/Inf in perplexity mode'


def test_dimpo_rejects_negative():
    try:
        DIMPO(n_components=1, max_iter=10, verbose=False).fit(np.array([[1.0, -1.0], [2.0, 3.0]]))
...
if __name__ == '__main__':
    all_tests = [
        ...
        # end-to-end
        test_dimpo_fit_transform_shape, test_dimpo_cost_decreases,
        test_dimpo_with_momentum, test_dimpo_with_early_exaggeration,
        test_dimpo_with_gamma, test_dimpo_perplexity_mode, test_dimpo_rejects_negative,
    ]
    for t in all_tests:
        t()
