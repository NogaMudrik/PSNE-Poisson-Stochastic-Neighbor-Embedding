# -*- coding: utf-8 -*-
"""
DIMPO: DIMensionality reduction for POisson data.
...
Core module containing all algorithm components:
    ...
    - DIMPO class (sklearn-style: fit / fit_transform)

Usage:
    from dimpo.dimpo_core import DIMPO
    model = DIMPO(n_components=3, s_mode='weight_exp', weight_exp=1.0)
...
#from dimpo.dimpo_utils import assert_finite, assert_shape, assert_nonnegative, assert_probability_matrix
#from dimpo.dimpo_config import get_default_optimizer_params
from dimpo_utils import assert_finite, assert_shape, assert_nonnegative, assert_probability_matrix
from dimpo_config import get_default_optimizer_params
...
def optimize(S, y_init, optimizer_params):
    ...
    optimizer_params : dict — see dimpo_config.get_default_optimizer_params()
...
class DIMPO:
    ...
    def fit(self, X):
        ...
        if self.verbose:
            print('DIMPO: N=%d features, T=%d samples, p=%d components' % (N, T, self.n_components))
