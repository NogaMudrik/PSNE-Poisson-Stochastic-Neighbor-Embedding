...
# author: noga mudrik
"""
PSNE Demo Script — Synthetic Poisson Data

Demonstrates PSNE on two synthetic Poisson datasets with known group structure:
  1. nonlinear_standard: 3-group
  2. nonlinear_xor:      4-group  variant (low counts, high sparsity)

Compares PSNE variants against baselines (PCA, t-SNE, and optionally all run_baselines methods).
...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'psne'))
from psne_utils import save_fig, assert_finite, assert_nonnegative, checkEmptyList
from psne_core import PSNE, compute_distance_matrix, compute_S
...
PSNE_W_MID = 1.0
PSNE_W_HIGH = 2.0
PSNE_GAMMA_SPARSE = 0.01
N_COMPONENTS = 2#  3  # 2 or 3

# --- What to run ---
RUN_PSNE_W_LOW = True
RUN_PSNE_W_MID = True
RUN_PSNE_W_HIGH = True
RUN_PSNE_SPARSE = True   # group lasso variant (gamma > 0)
...
def run_demo_dataset(X, labels, n_groups, dataset_name, path_save, params_dict, t_values=None):
    """
    Runs PSNE variants + baselines on X, saves figures and embeddings.
    ...
    if RUN_PSNE_W_MID:
        print('\n--- PSNE ($w$=%.1f) ---' % PSNE_W_MID)
        model = PSNE(n_components=N_COMPONENTS, s_mode='weight_exp', weight_exp=PSNE_W_MID,
                      eta=PSNE_ETA, max_iter=PSNE_MAX_ITER, gamma=0.0,
                      use_momentum=True, use_early_exaggeration=True,
                      exaggeration_iters=100, exaggeration_factor=4.0,
                      verbose=True, random_state=SEED)
        t0 = time.time()
        embeddings['psne_wexp'] = model.fit_transform(X)
        
        models['psne_wexp'] = model
        models['psne_wexp'].runtime_sec = time.time() - t0

    if RUN_PSNE_W_LOW:
        print('\n--- PSNE ($w$=%.1f) ---' % PSNE_W_LOW)
        model = PSNE(n_components=N_COMPONENTS, s_mode='weight_exp', weight_exp=PSNE_W_LOW,
                      eta=PSNE_ETA * 2, max_iter=PSNE_MAX_ITER, gamma=0.0,
                      use_momentum=True, use_early_exaggeration=True,
                      exaggeration_iters=100, exaggeration_factor=4.0,
                      verbose=True, random_state=SEED)
        t0 = time.time()
        embeddings['psne_wexp_low'] = model.fit_transform(X)       
        
        models['psne_wexp_low'] = model
        models['psne_wexp_low'].runtime_sec = time.time() - t0

    if RUN_PSNE_W_HIGH:
        print('\n--- PSNE ($w$=%.1f) ---' % PSNE_W_HIGH)
        model = PSNE(n_components=N_COMPONENTS, s_mode='weight_exp', weight_exp=PSNE_W_HIGH,
                      eta=PSNE_ETA, max_iter=PSNE_MAX_ITER, gamma=0.0,
                      use_momentum=True, use_early_exaggeration=True,
                      exaggeration_iters=100, exaggeration_factor=4.0,
                      verbose=True, random_state=SEED)
        t0 = time.time()
        embeddings['psne_wexp2'] = model.fit_transform(X)
        models['psne_wexp2'] = model
        models['psne_wexp2'].runtime_sec = time.time() - t0

    if RUN_PSNE_SPARSE:
        print('\n--- PSNE (group lasso, gamma=%.3f) ---' % PSNE_GAMMA_SPARSE)
        model = PSNE(n_components=N_COMPONENTS, s_mode='weight_exp', weight_exp=PSNE_W_MID,
                      eta=PSNE_ETA, max_iter=PSNE_MAX_ITER, gamma=PSNE_GAMMA_SPARSE,
                      use_momentum=True, use_early_exaggeration=True,
                      exaggeration_iters=100, exaggeration_factor=4.0,
                      verbose=True, random_state=SEED)
        t0 = time.time()
        embeddings['psne_sparse'] = model.fit_transform(X)
        models['psne_sparse'] = model
        models['psne_sparse'].runtime_sec = time.time() - t0
    ...
    np.save(os.path.join(path_save, 'psne_timing.npy'), timing_dict)
    ...
    method_titles.update({
        'psne_wexp':    r'PSNE ($w$=%.1f)' % PSNE_W_MID,
        'psne_wexp_low':r'PSNE ($w$=%.1f)' % PSNE_W_LOW,
        'psne_wexp2':   r'PSNE ($w$=%.1f)' % PSNE_W_HIGH,
        'psne_sparse':  r'PSNE ($\gamma$=%.2f)' % PSNE_GAMMA_SPARSE,
    })
    ...
    # ----------------------------------------------------------------
    # Cost convergence — PSNE only
    # ----------------------------------------------------------------
    psne_models = {k: v for k, v in models.items()}
    if len(psne_models) > 0:
        n_psne = len(psne_models)
        fig, axes = plt.subplots(1, n_psne, figsize=(5 * n_psne, 4))
        if n_psne == 1:
            axes = [axes]
        else:
            axes = axes.flatten()
        for ax, (name, model) in zip(axes, psne_models.items()):
...
params_nl = {
    ...
    'seed': SEED,
    'psne_eta': PSNE_ETA,
    'psne_max_iter': PSNE_MAX_ITER,
    'psne_w_low': PSNE_W_LOW,
    'psne_w_mid': PSNE_W_MID,
    'psne_w_high': PSNE_W_HIGH,
    'psne_gamma_sparse': PSNE_GAMMA_SPARSE,
